...
#     n_t = torch.sqrt(FpFmZ.shape[1])
    E2 = torch.exp(-T/T2)
    E1 = torch.exp(-T/T1)
    E = torch.stack((E2,E2,E1)).transpose(1,0).flatten().float()
    FpFm = FpFmZ.transpose(1,0).flatten().float()
#     FpFmZ2 = (FpFm*E).reshape(320*320,3).transpose(1,0)
    nt = np.int((E.shape[0])/3)
    FpFmZ2 = (FpFm*E).reshape(nt,3).transpose(1,0)
...
def SE_sim(angle_ex_rad, angles_rad, TE, Im, TR, B1=1.):
    # Im 4-D tensor (Batch size, channels(T1,T2,pd),X,Y)
    n_batch = Im.shape[0]
    n_size = Im.shape[2]
    T1 = Im[:,0,:,:].flatten()
    T2 = Im[:,1,:,:].flatten()
    ...
    Mxy_1 = Mxy * par.float()*pd.float()
    for t in range(n_batch):
        if t == 0:
            re = Mxy_1[:,t*n_size*n_size:(t+1)*n_size*n_size].reshape(1,1,n_size,n_size)
        else:
            print(re.shape)
            re = torch.cat((re,Mxy_1[:,t*n_size*n_size:(t+1)*n_size*n_size].reshape(1,1,n_size,n_size)),0)
#     print(type(Mxy))
