...
class VDE(BaseEstimator, nn.Module):
    ...
    """
    def __init__(self, input_size, lag_time=10, encoder_size=10, batch_size=100,
...
if __name__ == "__main__":
    VDE_test = VDE(input_size=500, batch_size=256, encoder_size=100, n_epochs=10, cuda=True)
    test = torch.rand(1000,500)
    VDE_test.fit_transform(test)
    print(np.array(VDE_test.transform(test)).shape)